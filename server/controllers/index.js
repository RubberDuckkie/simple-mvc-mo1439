...
const searchNameDog = (req, res) => {
  ...
  return Dog.findByName(req.query.name, (err, doc) => {
    ...
    savePromise.catch( (errr) => res.status(500).json({errr}));

    // if a match, send the match back
    return res.json({ name: obj.name, breed: obj.breed, age: obj.age });
  });
};
